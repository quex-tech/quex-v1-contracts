...
def normalize_object(c, d):
    res = {}
    for f in dataclasses.fields(c):
        if dataclasses.is_dataclass(f.type):
            res[f.name] = normalize_object(f.type, d[f.name])
        elif f.type == bytes:
            res[f.name] = b64decode(d[f.name])
        elif f.type == int and isinstance(d[f.name], str):
            res[f.name] = int(b64decode(d[f.name]))
        else:
            res[f.name] = d[f.name]
    return c(**res)

def get_td_quote(quote_dict) -> TDQuote:
    quote = normalize_object(TDQuote, quote_dict)
    if quote.quote_signature_data.qe_certification_data.certification_data_type == 6:
        quote.quote_signature_data.qe_certification_data.certification_data = \
            normalize_object(QEReportCertificationData, quote.quote_signature_data.qe_certification_data.certification_data)
    qe_cert_data = quote.quote_signature_data.qe_certification_data.certification_data.qe_certification_data
    cert_chain_str = b64decode(qe_cert_data.certification_data)
    quote.quote_signature_data.qe_certification_data.certification_data.qe_certification_data.certification_data = cert_chain_str
    return quote
...
quote = get_td_quote(quote_dict)


td_quote_json = {
    "USER_DATA": f"0x{quote.quote_header.user_data.hex()}",
    "TEE_TCB_SVN": f"0x{quote.td_quote_body.tcb_svn.hex()}",
    "MRSEAM": f"0x{quote.td_quote_body.mrseam.hex()}",
    "MRSIGNERSEAM": f"0x{quote.td_quote_body.mrsignerseam.hex()}",
    "SEAMATTRIBUTES": f"0x{quote.td_quote_body.seamattributes.hex()}",
    "TDATTRIBUTES": f"0x{quote.td_quote_body.tdattributes.hex()}",
    "XFAM": f"0x{quote.td_quote_body.xfam.hex()}",
    "MRTD": f"0x{quote.td_quote_body.mrtd.hex()}",
    "MRCONFIGID": f"0x{quote.td_quote_body.mrconfigid.hex()}",
    "MROWNER": f"0x{quote.td_quote_body.mrowner.hex()}",
    "MROWNERCONFIG": f"0x{quote.td_quote_body.mrownerconfig.hex()}",
    "RTMR0": f"0x{quote.td_quote_body.rtmr0.hex()}",
    "RTMR1": f"0x{quote.td_quote_body.rtmr1.hex()}",
    "RTMR2": f"0x{quote.td_quote_body.rtmr2.hex()}",
    "RTMR3": f"0x{quote.td_quote_body.rtmr3.hex()}",
    "REPORT_DATA1": f"0x{quote.td_quote_body.reportdata[:32].hex()}",
    "REPORT_DATA2": f"0x{quote.td_quote_body.reportdata[32:].hex()}"
}

td_deploy_data = {
    "tdQuote": td_quote_json,
    "attestationKey": {
        "x": f"0x{quote.quote_signature_data.ecdsa_attestation_key.x.hex()}",
        "y": f"0x{quote.quote_signature_data.ecdsa_attestation_key.y.hex()}"
    },
    "qeAuthenticationData": f"0x{quote.quote_signature_data.qe_certification_data.certification_data.qe_authentication_data.data.hex()}",
    "quoteSignature": {
        "r": f"0x{quote.quote_signature_data.quote_signature.r.hex()}",
        "s": f"0x{quote.quote_signature_data.quote_signature.s.hex()}"
    }
}

# Insert platformCaCert, processorPckCert, qeReportData, qeReportSignature
qe_report = quote.quote_signature_data.qe_certification_data.certification_data.qe_report
qe_cert = quote.quote_signature_data.qe_certification_data.certification_data.qe_certification_data

td_deploy_data["qeReportData"] = {
    "CPUSVN": f"0x{qe_report.cpu_svn.hex()}",
    "MISCSELECT": f"0x{qe_report.miscselect.to_bytes(4, 'little').hex()}",
    "MRENCLAVE": f"0x{qe_report.mrenclave.hex()}",
    "attributes": f"0x{qe_report.attributes.hex()}",
    "MRSIGNER": f"0x{qe_report.mrsigner.hex()}",
    "ISVProdID": f"0x{qe_report.isv_prodID.to_bytes(2, 'little').hex()}",
    "ISVSVN": f"0x{qe_report.isv_svn.to_bytes(2, 'little').hex()}",
    "REPORT_DATA1": f"0x{qe_report.report_data[:32].hex()}",
    "REPORT_DATA2": f"0x{qe_report.report_data[32:].hex()}"
}

qe_report_signature = quote.quote_signature_data.qe_certification_data.certification_data.qe_report_signature

td_deploy_data["qeReportSignature"] = {
    "r": f"0x{qe_report_signature.r.hex()}",
    "s": f"0x{qe_report_signature.s.hex()}"
}

with open("td_quote_parsed.json", "w") as out_file:
    json.dump(td_deploy_data, out_file, indent=4)